...
foreach ($zip in $zips) {
    ...
    # --max-time 5400：宽限 90 分钟/文件。实测本机到 Gitee 链路被限速 ~34KB/s，62MB 需 ~30 分钟才发完，
    #            旧 1800s 会在「刚发完等服务器回包」瞬间被掐断（UPLOAD_SIZE 已满但 EXITCODE=28）。放大到 5400 让慢链跑完。
    #            （真·空转仍会被上面的 --speed-limit 30s 秒杀，放大上限不掩盖故障）
