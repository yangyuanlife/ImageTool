...
foreach ($zip in $zips) {
    ...
    # 后者会把含空格的路径重新解析，切出多余参数被 curl 当成非法 URL（报错 Bad hostname）。
    # --speed-limit 1 --speed-time 30：连续 30s 速率<1B/s 即中止 → 心跳还在跳就证明字节在流动（否则早崩了）
    # --max-time 5400：宽限 90 分钟/文件。兜底安全值（直连后一般很快跑完，放大上限不碍事；真·空转仍会被 --speed-limit 30s 秒杀）
    # --noproxy gitee.com：关键！若系统设了本地代理（https_proxy 环境变量），curl 会继承并把 Gitee 上传绕代理，
    #            实测 proxy 路径首页 HEAD 都要 22s+、大文件被严重限速（还触发 schannel 重协商）。
    #            Gitee 是国内站点，本不需要代理 → 显式 --noproxy 直连，速度恢复正常。
    # --http1.1：强制 HTTP/1.1，绕开 curl+Schannel 在 HTTP/2 下上传大文件时「连接建好但 0 字节发出、最终 timeout」的死锁
    #            （实测：小 GET 正常、大 POST 卡死 = 典型 HTTP/2+Schannel 上传 stall；降 1.1 后正常流式发送，UPLOAD_SIZE 满）
    # -w：结束记录实际上传量/速率/耗时，写日志便于判断是否网络本身慢
    $curlArgs = @('-s', '-S', '--connect-timeout', '30', '--max-time', '5400',
